...
public class Conductor : MonoBehaviour
{
    ...
    GameObject[][] notes;
    public int[,] spawnNotes = new int[,] { { 1, 2 }, { 3, 4 } };
    ...
    void Start()
    {
        ...
        songBeats = new float[notes.GetLength(0)];
        compLoops = new int[notes.GetLength(0)];
        loopAnalog = new float[notes.GetLength(0)];
        ...
        notePrefabs = new GameObject[notes.GetLength(0)]; //makes sure they match 
    }
        ...
        if (i <= notes.GetLength(0) - 4)
        {
            for (int j = 0; j < notePrefabs.Length; j++)
            {
                if (spawnNotes[i,j] == 1);
                {
                    ...
                    notes[i][j] = Instantiate(notePrefabs[j], transform.position, transform.rotation) as GameObject;
                    notes[i][j].transform.parent = cam;
