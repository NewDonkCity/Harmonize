...
public class Dash : MonoBehaviour
{
    ...
    void Dashing()
    {
        ...
        if ((Time.time - startTime) >= secPerBeat || buttonHeldDown == false)
            power = 0f;
        if (Input.GetKeyDown("a") || Input.GetKeyDown("d") || Input.GetKeyDown("w") || Input.GetKeyDown("s"))
        {
            startTime = Time.time;
        }
        if (Input.GetKey("a") && (Time.time - startTime) <= secPerBeat)
        {
            if ((Time.time - startTime) > secPerBeat)
                power = 0f;
            else
        ...
        else if (Input.GetKey("w") && (Time.time - startTime) <= (secPerBeat + 0.2f))
        {
            if ((Time.time - startTime) > secPerBeat)
                power = 0f;
            else
        ...
        else if (Input.GetKey("s") && (Time.time - startTime) <= (secPerBeat + 0.2f))
        {
            if ((Time.time - startTime) > secPerBeat)
                power = 0f;
            else
